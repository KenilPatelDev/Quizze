package com.appspouch.quizze.Results_section;
...
/*
    public void saveToExcel(String fileName) {
        ...
        //create row
        Keyboard.Row createRow = createSheet.createRow(0);
        ...
        while (i < this.testAdapter.dataList.size()) {
            int i2 = i + 1;
            Keyboard.Row createRow2 = createSheet.createRow(i2);
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (int i=0;i<result.size();i++){
                    if(dataSnapshot.child(result.get(i).userID).exists())
                        result.get(i).user=dataSnapshot.child(result.get(i).userID).getValue(Student.class);
                     else {
                        Student student = new Student();
                        student.setId("id");
                        result.get(i).user = student;
                    }
    ...
    class TestAdapter extends ArrayAdapter<TestResults> {
        ...
        public View getView(final int position, @Nullable View convertView, @NonNull ViewGroup parent) {
            ...
            if(dataList.get(position).user!=null)
                ((TextView)listItem.findViewById(R.id.item_textView)).setText(dataList.get(position).user.id);
          /*  else {
